...
class BoopGUI:
    ...
    def create_welcome_screen(self):
        ...
        self._mode_button(
            welcome_card, f"{ICON_PLAYER} Human vs AI", lambda: self.set_mode("human_ai")
        ).pack(fill=tk.X, pady=8)
        self._mode_button(
            welcome_card, f"{ICON_AI} AI vs AI", lambda: self.set_mode("ai_ai")
        ).pack(fill=tk.X, pady=8)
    ...
    def update_display(self):
        red_active = self.player_turn == "r" and self.game_running and self.winner is None
        blue_active = self.player_turn == "b" and self.game_running and self.winner is None

        red_title = f"{ICON_PLAYER} You" if self.mode == "human_ai" else f"{ICON_AI} Red AI"
        blue_title = f"{ICON_AI} Blue AI"

        self._update_player_card(
            self.red_card,
            title=red_title,
            fg=COLORS["red"],
            kittens=self.reserves["r"]["k"],
            cats=self.reserves["r"]["c"],
            detail=self._player_detail("r", red_active),
            active=red_active,
            active_bg=COLORS["red_soft"],
            is_human_card=self.mode == "human_ai",
            player_key="r",
        )
        self._update_player_card(
            self.blue_card,
            title=blue_title,
            fg=COLORS["blue"],
            kittens=self.reserves["b"]["k"],
            cats=self.reserves["b"]["c"],
            detail=self._player_detail("b", blue_active),
            active=blue_active,
        ...
        if self.winner is not None:
            ...
            self.status_label.config(text="Game stopped", fg=COLORS["danger"])
        elif self.mode == "ai_ai":
            if self.player_turn == "r":
                self.status_label.config(text=f"{ICON_AI} Red AI is playing...", fg=COLORS["red"])
            else:
                self.status_label.config(text=f"{ICON_AI} Blue AI is playing...", fg=COLORS["blue"])
        else:
            if self.player_turn == "r":
                self.status_label.config(text=f"{ICON_PLAYER} You are playing now", fg=COLORS["good"])
            else:
                self.status_label.config(text=f"{ICON_AI} Blue AI is playing...", fg=COLORS["blue"])
    ...
    def _draw_normal_board(self):
        for r in range(BOARD_SIZE):
            for c in range(BOARD_SIZE):
                cell = self.board[r, c]
                style = PIECE_STYLES[cell]
                self.cells[r][c]["frame"].config(bg=style["bg"])
                self.cells[r][c]["label"].config(text=style["text"], bg=style["bg"], fg=style["fg"])
    ...
    def _draw_winner_board(self):
        ...
        for r in range(BOARD_SIZE):
            for c in range(BOARD_SIZE):
                ...
                if cell == ".":
                    frame.config(bg=winner_bg)
                    label.config(text="", bg=winner_bg, fg=COLORS["text"])
                elif cell in (winner_piece, winner_kitten):
                    icon = ICON_CAT if cell.endswith("c") else ICON_KITTEN
                    frame.config(bg=winner_strong)
                    label.config(text=icon, bg=winner_strong, fg="white")
                else:
                    icon = ICON_CAT if cell.endswith("c") else ICON_KITTEN
                    frame.config(bg=winner_bg)
                    label.config(text=icon, bg=winner_bg, fg=loser_strong)
    ...
    def _update_player_card(
        self, card, title, fg, kittens, cats, detail, active, active_bg, is_human_card, player_key
    ):
        bg = active_bg if active else COLORS["card"]
        is_red_card = title.endswith("You") or title.endswith("Red AI")
        is_blue_card = title.endswith("Blue AI")

        if self.winner == "r" and is_red_card:
            bg = COLORS["red_soft"]
        if self.winner == "b" and is_blue_card:
            bg = COLORS["blue_soft"]

        card["frame"].config(bg=bg)
        card["title"].config(text=title, fg=fg, bg=bg)
        card["counts"].config(
            text=f"{ICON_KITTEN} Kittens: {kittens}    {ICON_CAT} Cats: {cats}",
            fg=COLORS["text"],
            bg=bg,
        )
        card["detail"].config(text=detail, fg=COLORS["muted"], bg=bg)
        card["selector"].config(bg=bg)

        if is_human_card and self.winner is None:
            self._ensure_selector_buttons_visible(card)
            self._update_piece_selector(card, bg, player_key)
    ...
    def _update_piece_selector(self, card, bg, player_key):
        can_use_kitten = self.reserves[player_key]["k"] > 0
        can_use_cat = self.reserves[player_key]["c"] > 0
